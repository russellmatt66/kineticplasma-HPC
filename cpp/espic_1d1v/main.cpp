...
int main(int argc, char* argv[])
{
    ...
    */
    ...
    cout << "Initial Particle Weighting" << endl;
    size_t routineFlag = ParticleWeight(W, particle_x, x_grid, rho, particle_indices, Q_net); // compute rho as a side-effect
    /*
    Process routineFlag
    */
    cout << "Initial Particle Weighting sucessful" << endl;
    // cout << "Recording initial phasespace" << endl;
    // routineFlag = CollectParticleData(particle_data,particle_string,t,n,particle_x,particle_v,particle_E);
    ...
    // // Construct A
    cout << "Building linear system for poissons equation" << endl;
    routineFlag = BuildSparseLapl(A,dx);
    /*
    Process routineFlag 
    */
    // cout << "Linear system built successfully" << endl;    

    // Given initial charge configuration, obtain initial potential
    cout << "Initial Field Solve" << endl;
    // routineFlag = FieldSolve(rho,dx,phi,E_grid);
    routineFlag = FieldSolveMatrix(A,dx,rho,phi,E_grid);
    /*
    Process routineFlag 
    */
    cout << "Initial Field Solve successful" << endl;

    cout << "Grid finished initializing" << endl;  
    routineFlag = CollectGridData(grid_data,grid_string,t,n,x_grid,rho,phi,E_grid,Q_net);
    routineFlag = CollectEnergyHistory(energy_data,energy_string,t,n,dt,E_grid,particle_v,x_grid);
    // Weight the initial grid field to the particles
    cout << "Initial Force Weighting" << endl;
    routineFlag = ForceWeight(W,particle_indices,E_grid,x_grid,particle_x,particle_E);  
    /*
    Process routineFlag
    */
    cout << "Initial Force Weighting successful" << endl;
    cout << "Recording initial phasespace" << endl;
    routineFlag = CollectParticleData(particle_data,particle_string,t,n,particle_x,particle_v,particle_E,particle_indices,x_grid);

    // Half-step backwards, then push the particles
    // Need to normalize v, [dt*particle_E] = [dt]
    for (size_t in = 0; in < particle_v.num_rows(); in++){ // Half-step backwards in velocity
        particle_v(in) = particle_v(in) - 0.5*dt*particle_E(in); 
    }

    cout << "Initial Particle Push" << endl;
    routineFlag = ParticlePush(particle_E,dt,x_grid,particle_x,particle_v);
    /*
    Process routineFlag
    */
    cout << "Initial Particle Push successful" << endl;
    
    // PIC Algorithm
    for (size_t it = 1; it < Nt; it++){
        cout << "Starting timestep " << it << endl;
        t += dt;
        // Charge Weight
        // cout << "Weighting Particles" << endl;
        routineFlag = ParticleWeight(W, particle_x, x_grid, rho, particle_indices, Q_net);  
        // Field Solve
        // cout << "Solving fields" << endl;
        // routineFlag = FieldSolve(rho,dx,phi,E_grid);
        routineFlag = FieldSolveMatrix(A,dx,rho,phi,E_grid);  
        // Force Weight
        // cout << "Weighting force" << endl;
        routineFlag = ForceWeight(W,particle_indices,E_grid,x_grid,particle_x,particle_E);
        cout << "Recording grid state" << endl;
        routineFlag = CollectGridData(grid_data, grid_string, t, it, x_grid, rho, phi, E_grid, Q_net);
        // Particle Push
        // cout << "Pushing particles" << endl;
        cout << "Recording energy history" << endl;
        routineFlag = CollectEnergyHistory(energy_data,energy_string,t,it,dt,E_grid,particle_v,x_grid);
        cout << "Recording particle states" << endl;
        routineFlag = CollectParticleData(particle_data,particle_string,t,it,particle_x,particle_v,particle_E,particle_indices,x_grid);
        routineFlag = ParticlePush(particle_E,dt,x_grid,particle_x,particle_v);
        // Collect Data
        cout << "timestep " << it << " complete" << endl;
